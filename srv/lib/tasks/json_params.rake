...
namespace :json_params do
  ...
  task :generate => :environment do
    ...
    create(Experience, "#{Rails.root}/app/controllers/params/my_experience.rb",
      [],
      nil,
      nil,
      'MyExperience'
    )
    create(Experience, "#{Rails.root}/app/controllers/params/friend_experience.rb",
      [],
      nil,
      {name: 'string'},
      'FriendExperience'
  ...
  private
    ...
    def create(object, file_path, exclusion_list=[], special_case_list=nil, additional_list=nil, alias_name=nil)
      val = ''
      if (object.instance_of?(String))
        # Custom Definitions
        additional_list.each do |k, v|
        ...
        additional_list && additional_list.each do |k, v|
          val << "# @attr [#{v}] #{k.to_s.camelize(:lower)}\n"
        end
      end
...
#
# @name #{name}StrongParameter
#
#{val.chomp}
class Params::#{name}; end
EOS
