...
RSpec.describe "Api::Search::Usernames", type: :request do
  ...
  describe "GET /api/search/usernames" do
    context "when user input search_key" do
      it "works!" do
        get api_search_usernames_path, params: { keyword: view_user.name[-1] } , headers: { 'Authorization' => "#{current_user.access_token}" }
    ...
    context "when search_key is empty" do
      it "can't work" do
        get api_search_usernames_path, params: { keyword: {} } , headers: { 'Authorization' => "#{current_user.access_token}" }
